...
> **状态**: 已实现（`autoload/quickui/dialog.vim`）
...
| `vertical` | Number | 否 | `auto` | 布局方式：`0` 强制水平，`1` 强制垂直，不指定时自动选择（选项总宽度超出内容区宽度时自动切换垂直） |

- 可聚焦
- 占用 1 行（水平排列）
- 布局：`prompt  (*) Dev  ( ) QA  ( ) PM`
- 若选项总宽度超出对话框内容区宽度，自动切换为垂直布局（每个选项独占一行，占用多行），确保所有选项始终可见。调用方也可通过 `vertical: 1` 选项主动指定垂直布局
...
- **name 唯一性**：所有带 `name` 字段的控件，其 `name` 值必须唯一（`name` 用作返回值字典的键）。`s:parse_items()` 检测到重复 `name` 时 `echoerr` 报错并中止。
- **button 默认 name**：如果存在多个 button 控件，每个都必须显式指定不同的 `name`，否则默认的 `'button'` 会导致冲突。
- **Enter 行为一致性**：在所有可交互控件（input、radio、check、button）上按 Enter 都会确认对话框，返回 `button='enter'`（input/radio/check 上）或该 button 控件的 `name`（button 控件上）。

### 边界情况

- **空 items 列表**：`quickui#dialog#open([], opts)` 返回 `{'button': '', 'button_index': -1}`，不创建窗口。
- **全是 label**：无可聚焦控件时，焦点链为空，主循环等待 ESC/Ctrl-C 取消。返回 `{'button': '', 'button_index': -1}`。
- **无 button 控件**：用户通过 Enter（从任意可交互控件）或 ESC 关闭对话框。Enter 返回 `{'button': 'enter', 'button_index': 0}`，ESC 返回 `{'button': '', 'button_index': -1}`。
...
- 所有命名控件的值（以 `name` 为键）
- `button` 字段标识哪个 button 控件被激活，或 `'enter'` 表示 Enter 键确认
- `button_index` 字段标识该控件内第几个按钮（1-based，与 `quickui#confirm#open()` 一致）
...
**用户按 Enter 确认**（从任意 input/radio/check 控件按 Enter）:
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `button` | String | 被激活的 button 控件的 `name`；`'enter'` 表示从任意可交互控件按 Enter 确认；取消时为空字符串 |
| `button_index` | Number | 被激活按钮在其控件 `items` 中的索引（1-based，与 `quickui#confirm#open()` 一致）；Enter 确认时为 `0`（虚拟确认按钮）；取消时为 `-1` |
...
连续的带 `prompt` 字段的控件（input、radio、设置了 prompt 的 check），中间不被不含 prompt 的**交互控件**打断，形成一个"对齐组"。组内所有 prompt 按最长的 prompt 宽度 + 2 对齐：
...
label（静态文本）**不打断**对齐组——它是说明性内容，不影响表单字段的视觉连贯性。只有不含 prompt 的交互控件（如无 prompt 的 check）才打断对齐组。
...
| `Tab` | 焦点前进（循环） |
| `Shift-Tab` | 焦点后退（循环） |
| `Up` | 焦点前进（纵向导航，与 Tab 等价但在视觉方向上更直觉） |
| `Down` | 焦点后退（纵向导航，与 Shift-Tab 等价） |
...
| `Enter` | **确认对话框**：关闭对话框，返回结果标记为正常确认（`button='enter'`、`button_index=0`） |
| `Ctrl+Up` / `Ctrl+Down` | 浏览历史记录（委托给 `rl.feed()`，映射为 `<C-p>`/`<C-n>`） |

说明：input 控件聚焦时接管大部分按键，只有 Tab/S-Tab/Enter/ESC 走全局处理。Enter 在 input 上直接确认对话框，这是最自然的交互——用户填写完最后一个输入框后按 Enter 即可提交。历史浏览使用 Ctrl+Up/Ctrl+Down（对应 readline 的 `<C-p>`/`<C-n>`），释放 Up/Down 用于控件间纵向导航。
...
| `Left` / `h` | 选择上一个选项（循环） |
| `Right` / `l` | 选择下一个选项（循环） |
| `Space` | 选择下一个选项（与 Right 一致，提供 Space 操作的一致体验） |
| `Enter` | **确认对话框**：关闭对话框，返回结果标记为正常确认（`button='enter'`、`button_index=0`） |
...
当焦点在 input 控件上时，所有可打印字符都直接传递给 readline，**不进行快捷键检查**。这是为了避免用户在 input 中输入包含快捷键字母的文本时（如输入 "hello" 时 "o" 恰好是 `&OK` 按钮的快捷键）被错误拦截。快捷键仅在焦点位于 radio、check、button 控件时才被检查。

按键分发优先级：
1. 全局键（ESC/Tab/S-Tab/Mouse）—— 最高优先级，所有控件类型
2. 若焦点在 input：所有剩余按键直接给 readline（**跳过快捷键**）
3. 若焦点在 radio/check/button：先检查快捷键，再走控件专属处理

**冲突处理**：`s:build_keymap()` 按 items 顺序注册快捷键。若同一字符被多个控件使用，`s:build_keymap()` 应 `echoerr` 报错并指出冲突的控件名称，与 name 唯一性检查同级别——快捷键冲突是调用方的错误，不应默默覆盖。
...
| 控件 | 聚焦态 | 未聚焦态 |
|------|--------|----------|
| input 文本区域 | `QuickInput` | `QuickOff`（`overlay(QuickInput, QuickDefaultDisable)`：保留 input 背景色 + 使用主题的 disable 前景色，既能看到输入框，又与聚焦态有明显区分） |
...
`QuickOff` 在 `s:hl_prepare()` 中动态生成，不再使用静态链接：

```vim
" 通过 overlay 组合：取 QuickInput 的背景色 + QuickDefaultDisable 的前景色
call quickui#highlight#overlay('QuickOff', 'QuickInput', 'QuickDefaultDisable')
```

各主题的实际效果：

| 主题 | 聚焦 input (QuickInput) | 未聚焦 input (QuickOff) | 窗口背景 (QuickBG) |
|------|------------------------|------------------------|---------------------|
| borland | 白字 #e4e4e4 + 蓝底 #0000a0 | 灰字 #75715e + 蓝底 #0000a0 | 黑字 + 灰底 #c0c0c0 |
| gruvbox | 米色 #ebdbb2 + 深色 #282828 | 灰棕 #928374 + 深色 #282828 | 米色 + 棕色 #504945 |
| solarized | 灰蓝 #839496 + 深青 #002b36 | 石板灰 #586e75 + 深青 #002b36 | 深色 + 灰蓝 #839496 |
...
let hwnd = {
    ...
    \ 'h': 20,                 " 内容区高度
    \ 'keymap': {...},         " 快捷键映射表
    \ 'exit': 0,               " 是否退出主循环（ESC/Ctrl-C/关闭按钮/Enter/按钮激活时置 1）
    \ 'result': {...},         " 最终返回值 Dict（退出时填充）
    \ }
```
...
quickui#dialog#open(items, opts)
  ...
  └── 主事件循环
        ...
        └── s:handle_key(hwnd, ch)
              ├── 全局键判断：ESC/Ctrl-C/Tab/S-Tab/Up/Down/Mouse/快捷键/Enter
              │     Enter：构建 result（button='enter' 或当前 button name），置 exit=1
              │     Up/Down：焦点前进/后退（同 Tab/S-Tab）
              ├── 获取当前焦点控件
              └── 调用对应的 s:handle_<type>(hwnd, control, ch)
                    ├── s:handle_input()  → rl.feed(ch) / Ctrl+Up/Down→历史
                    ├── s:handle_radio()  → 切换选项
                    ├── s:handle_check()  → 切换状态
...
function! s:render_input(hwnd, control, focused)
    ...
    if a:focused
        ...
        call win.set_line(y, line, 0)  " refresh=0，由 render_all 统一刷新
    else
        " 未聚焦：显示静态文本，使用 QuickOff 高亮
        " QuickOff = overlay(QuickInput, QuickDefaultDisable)：
        " 保留 input 蓝底（看得到框）+ 使用 disable 灰色前景（区分焦点）
        let text = rl.update()
        " 截断超出宽度的部分
        ...
...
> **关键实现细节：`win.update()` 刷新**
>
> 所有 render 函数使用 `win.set_line(y, line, 0)` 写入文本，`refresh=0` 参数表示只更新内存中的 `self.text[]` 数组，不立即调用 `setbufline()` 写入 buffer。这是为了在 `syntax_begin()`/`syntax_end()` 之间批量操作时避免逐行刷新的开销。
>
> `s:render_all()` 在 render 循环结束后、`syntax_end()` 之前调用 `win.update()` 一次性将所有行刷新到 buffer：
>
> ```vim
...
>     ...render each control...
> endfor
> call win.update()       " 刷新 self.text[] → buffer
> call win.syntax_end()   " 应用语法高亮
> ```
...
function! s:handle_mouse(hwnd) abort
    ...
    if s:has_nvim == 0
        ...
        " Vim popup 的 pos.column/pos.line 已经是内容区相对坐标
        " 只需做 1-based → 0-based 转换
        let x = pos.column - 1
        let y = pos.line - 1
    else
...
1. **radio 垂直布局** — 已内置 `vertical` 选项和自动切换机制（选项溢出时自动垂直），此条从预留列表移至已实现
...
以下是实际实现过程中对本设计方案的修订，供后续迭代参考。

### 修订 1：快捷键与 input 的优先级冲突

**问题**：原设计中快捷键"全局有效"，但在 input 焦点时，用户输入的字符可能恰好匹配按钮快捷键（如输入 "hello" 中的 `o` 匹配 `&OK` 的 `o`），导致 dialog 被意外关闭。

**修正**：`s:handle_key()` 中增加焦点类型判断。当焦点在 input 上时，跳过 `s:dispatch_hotkey()` 调用，所有可打印字符直接传递给 readline。快捷键仅在 radio/check/button 焦点时检查。

### 修订 2：渲染函数需要 win.update()

**问题**：所有 render 函数使用 `win.set_line(y, line, 0)` 写入文本（`refresh=0` 只更新内存），但 `s:render_all()` 中缺少 `win.update()` 调用，导致 buffer 内容不刷新。表现为：input 文字不可见，radio `(*)` 标记不更新，check `[x]` 标记不更新。

**修正**：在 `s:render_all()` 的 render 循环结束后、`win.syntax_end()` 之前加入 `call win.update()`。

### 修订 3：QuickOff 高亮组改用 overlay

**问题**：初版使用 `make_underline('QuickOff', 'QuickBG')` 生成未聚焦 input 的高亮，但 `QuickOff` 的前景色/背景色与窗口背景 `QuickBG` 完全相同，input 框与背景融为一体不可见。改为 `overlay('QuickOff', 'QuickInput', 'QuickBG')` 后，黑色前景在蓝色背景上过于生硬。

**最终方案**：`overlay('QuickOff', 'QuickInput', 'QuickDefaultDisable')`。保留 `QuickInput` 的背景色（蓝底=可见的输入框），使用 `QuickDefaultDisable` 的前景色（主题配套的灰色调），既保持"框"的视觉，又与聚焦态有明显区分。

### 修订 4：Vim 鼠标坐标无需减去 padding

**问题**：原设计假设 Vim popup 的 `getmousepos().column` 包含 border+padding 偏移，需要减去 `padding_left`。但实际测试发现 Vim popup 的 `pos.column`/`pos.line` 已经是内容区相对坐标（与 confirm.vim 行为一致）。

**修正**：`s:handle_mouse()` 中 Vim 分支改为 `let x = pos.column - 1`（仅做 1-based → 0-based 转换）。

### 修订 5：input 历史记录保存

**问题**：dialog 关闭时保存 input 历史，但遗漏了 `rl.history_save()` 调用，导致当前输入未写入历史列表。

**修正**：在保存历史前先调用 `call ctrl.rl.history_save()`。

### 修订 6：平台检测变量清理

`s:has_popup` 变量在实际实现中未被使用（dialog 通过 `quickui#window` 间接使用 popup/floating API，不需要直接检测），已移除。仅保留 `s:has_nvim`。
