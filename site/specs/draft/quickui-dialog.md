...
- **Enter 行为一致性**：在所有可交互控件（input、radio、check、button）上按 Enter 都会确认对话框，返回 `button=''`、`button_index=0`（input/radio/check 上）或 `button=控件name`、`button_index=N`（button 控件上）。
...
- **无 button 控件**：用户通过 Enter（从任意可交互控件）或 ESC 关闭对话框。Enter 返回 `{'button': '', 'button_index': 0}`，ESC 返回 `{'button': '', 'button_index': -1}`。
...
| `Enter` | **确认对话框**：关闭对话框，返回结果标记为正常确认（`button=''`、`button_index=0`） |
...
quickui#dialog#open(items, opts)
  ...
  └── 主事件循环
        ...
        └── s:handle_key(hwnd, ch)
              ...
              │     Enter：构建 result（button='' 或当前 button name），置 exit=1
